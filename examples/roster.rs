...
fn main() {
	println!("  team {:>width$} port flags signature", "path", width = NAME_FIELD_WIDTH);
	println!("------ {:->width$}---- ----- ---------", " ", width = NAME_FIELD_WIDTH + 1);

	let team_list = ROSTER.get_app_list().expect("Unexpected error getting the list of teams");
	for team in team_list {
		...
		println!("{:>6} {:>34}{:>5} {:>5} ({})", app_info.team, path, app_info.port, app_info.flags, app_info.signature);
	}
}
